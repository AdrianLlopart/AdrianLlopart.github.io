...
const MediaDisplay: React.FC<MediaLinks> = ({ videoUrl, websiteUrl, pdfUrl }) => {
  const [fullScreenMedia, setFullScreenMedia] = useState<{ type: 'video' | 'pdf', url: string } | null>(null);

  if (!videoUrl && !websiteUrl && !pdfUrl) return null;
      ...
      {fullScreenMedia && (
        ...
        >
          <Dialog.Header id="media-dialog-header">
            {fullScreenMedia.type === 'video' ? 'Video' : 'PDF'}
          </Dialog.Header>
